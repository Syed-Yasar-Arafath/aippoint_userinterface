...
const Header: React.FC<HeaderProps> = ({ title, userProfileImage, path }) => {
  ...
  const connectNotification = (userId: any, dbName: any) => {
    ...
    // const ws = new WebSocket(`ws://https://aippoint.ai/parseez-django-service/ws/notifications/${userId}/`)
    if (!dbName) return
    const ws = new WebSocket(
      `ws://https://aippoint.ai/parseez-django-service/ws/notifications/${userId}/`,
    )
    // const ws = new WebSocket(
