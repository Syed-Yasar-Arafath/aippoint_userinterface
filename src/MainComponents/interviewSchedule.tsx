...
import { Button, FormControl, MenuItem, Select, Switch, useTheme } from '@mui/material'
import React, { ChangeEvent, useEffect, useState } from 'react'
import { loaderOff, loaderOn, openSnackbar } from '../redux/actions'
import { generateQuestions, getResumeById, postResume } from '../services/ResumeService'
...
const InterviewScheduler: React.FC = () => {
  ...
  const handleCheckboxClick = (
    ...
    if (isChecked) {
      if (id && email && email !== 'Missing') {
        setResumeId((prevIds) => [...prevIds, id])
      } else {
        // dispatch(openSnackbar('Email is mandatory', 'red'))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response?.message === 'Resumes processed successfully') {
        dispatch(openSnackbar(t('successPleaseProceedSnackbar'), 'dodgerblue'))
      } else {
        dispatch(
